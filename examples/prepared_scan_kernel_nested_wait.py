"""Prepared nested-kernel prepared-runtime example with a fixed core-side wait.

This is a timing-oriented counterpart to
``prepared_scan_kernel_nested_ttl.py``. It keeps the same nested prepared
...
PreparedScanKernelNestedWait = make_fragment_prepared_scan_exp(
    PreparedKernelNestedWaitFragment,
    lambda fragment: ScanRequest.explicit(
        [fragment.outer],
        [[1.0]],
        metadata={"demo_name": "prepared_scan_kernel_nested_wait"},
    ),
)
