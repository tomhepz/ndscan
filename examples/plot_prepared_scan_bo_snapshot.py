"""Plot a NUBO-style GP corner plot from a prepared-runtime HDF5 snapshot.

Usage:

    python examples/plot_prepared_scan_bo_snapshot.py <snapshot.h5> [--site path/to/site]
...
The plotter expects a prepared-runtime site whose ``scan.point_policy`` metadata looks
like:
...
try:
    ...
    raise ImportError(
        "plot_prepared_scan_bo_snapshot requires the optional Bayesian optimisation "
