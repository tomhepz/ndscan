"""Prepared-runtime Bayesian optimisation example with structured exploration phases.

This example shows the intended layering for more complicated adaptive search:

- the prepared runtime still owns batching, persistence, and nested scan structure,
...
try:
    ...
    raise ImportError(
        "prepared_scan_bayesian_optimisation requires the optional Bayesian "
...
def _make_request(fragment: MultiWellSurfaceFragment) -> ScanRequest:
    ...
    return ScanRequest(
        ...
        metadata={"demo_name": "prepared_scan_bayesian_optimisation"},
    )


PreparedScanBayesianOptimisation = make_fragment_prepared_scan_exp(
    MultiWellSurfaceFragment,
    _make_request,
)

PreparedScanBayesianOptimisationDashboard = make_fragment_prepared_dashboard_scan_exp(MultiWellSurfaceFragment)
