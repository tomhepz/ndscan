"""Prepared nested-kernel prepared-runtime example.

This is the compiler-friendly counterpart to
``prepared_scan_nested_p_variation.py``. The scan shape is intentionally similar:
...
PreparedScanKernelNested = make_fragment_prepared_scan_exp(
    PreparedKernelNestedVariationFragment,
    lambda fragment: ScanRequest.explicit(
        [fragment.outer],
        [[1.0]],
        metadata={"demo_name": "prepared_scan_kernel_nested"},
    ),
)
