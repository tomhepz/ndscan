"""Prepared nested-kernel host-runtime example with a real TTL leaf.

This is the hardware-oriented counterpart to
``host_runtime_prepared_kernel_nested.py``:
...
HostRuntimePreparedKernelNestedTtl = make_fragment_prepared_scan_exp(
    ...
    lambda fragment: ScanRequest.explicit(
        [fragment.outer],
        [[1.0]],
        metadata={"demo_name": "host_runtime_prepared_kernel_nested_ttl"},
    ),
)
