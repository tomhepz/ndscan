"""AMO-style reparameterisation examples for the host runtime.
...
- ``HostRuntimeAomMappedLogicalAxes`` uses ad hoc ``ScanVariable`` objects plus
  ``ParameterMapping`` in a code-first request.
- ``HostRuntimeAomWrapperRebind`` uses a reusable wrapper fragment and
  ``rebind_param(...)``.
- ``HostRuntimeAomDashboardRebind`` exposes only the hardware-facing parameters and is
...
HostRuntimeAomWrapperRebind = make_fragment_prepared_scan_exp(
    ...
    lambda fragment: ScanRequest.linear(
        ...
        stop=LASER_DETUNING_SCAN_MAX,
        num_points=LASER_DETUNING_NUM_POINTS,
        metadata={"demo_name": "host_runtime_aom_reparameterisation_wrapper"},
    ),
)
...
class LegacyManualAomWrapperFragment(ExpFragment):
    ...
    hoc than the host-runtime ``ParameterMapping`` path above.
    """
...
1. Open ``HostRuntimeAomDashboardRebind`` in the ARTIQ dashboard.
