"""Prepared-runtime fixed-parameter traceability example.
...
- a nested child scan site launched through a prepared child-scan handle.
...
class FixedParameterParentFragment(ExpFragment):
    ...
    def build_fragment(self):
        ...
        self.inner_scan = prepare_child_scan(self, self.child, name="inner")
        self.setattr_result("total", FloatChannel)

    def run_once(self):
        self.inner_scan.configure(
            ScanRequest.explicit(
                [self.child.x],
                [[self.outer.get()], [self.outer.get() + 1.0]],
            )
        )
        self.inner_scan.execute()
        child_result = self.inner_scan.inspect()
        self.total.push(self.multiplier.get() * sum(child_result.values[self.child.y]))


PreparedScanFixedParametersNested = make_fragment_prepared_scan_exp(
    FixedParameterParentFragment,
    lambda fragment: ScanRequest.explicit(
        [fragment.outer],
        [[10.0], [20.0]],
        metadata={"demo_name": "prepared_scan_fixed_parameters_nested"},
    ),
)
