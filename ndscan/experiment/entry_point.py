...
class FragmentScanExperiment(EnvExperiment):
    ...
    def prepare(self):
        ...
        explicit_param_targets = collect_explicit_param_targets(param_stores)
        self.fragment._activate_optional_param_relations(explicit_param_targets)
        activate_scan_arg_param_relations(
            self.fragment,
            self.args.get_scan_relation_specs(),
            explicit_param_targets,
            ScanSpecError,
        )
...
def run_fragment_once(
    ...
    fragment._activate_optional_param_relations(
        collect_explicit_param_targets(overrides)
    )
    fragment.prepare()
    try:
        while True:
