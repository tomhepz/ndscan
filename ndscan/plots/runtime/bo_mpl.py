"""Embedded matplotlib BO corner-plot helpers for the runtime viewer.

The existing pyqtgraph runtime viewer is a good fit for ordinary sampled scan sites.
For Gaussian-process optimisation sites, however, the most useful view is often a
root-level dashboard showing:

- posterior mean slices,
- posterior uncertainty slices,
- observed points coloured by provenance,
...
def _fit_exact_gp_model(
    ...
    *,
    ...
    FixedNoiseGaussianLikelihood, GaussianProcess, fit_gp, _ = (
...
def _posterior_mean_std(gp, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    if x.ndim == 1:
        x = x.reshape(1, -1)
    gp.eval()
...
def _surrogate_argmax(
    ...
    *,
    ...
    _, _, _, single = _require_bo_plot_dependencies()
...
def render_bo_site_to_figure(
    ...
    *,
    ...
    argmax_num_starts: int | None = None,
    ...
    try:
        ...
        fit_steps = int(
            fit_steps if fit_steps is not None else backend.get("fit_steps", 400)
        )
        fit_lr = float(fit_lr if fit_lr is not None else backend.get("fit_lr", 0.05))
        noise_floor = float(
            noise_floor
        ...
        argmax_num_starts = int(
            argmax_num_starts
            if argmax_num_starts is not None
            else backend.get("surrogate_num_starts", 20)
        )
        ...
        return (
            f"BO dashboard; {x_obs.shape[1]}D, {x_obs.shape[0]} observed point"
            f"{'' if x_obs.shape[0] == 1 else 's'}"
        )
...
def _plot_gp_corner(
    *,
    ...
    std_vmax = (
        np.concatenate(std_fields).max() if std_fields else float(np.max(objective_err))
    )

    axes = figure.subplots(dim, dim, squeeze=False)
    ...
    figure.suptitle(f"{title}: GP refit from live snapshot")
    figure.tight_layout()


class BoCornerPlotWidget(QtWidgets.QWidget):
    """Simple embedded matplotlib widget for root-level BO corner plots."""
    ...
    def render_site(self, site: ScanSiteData) -> str:
        ...
        status = render_bo_site_to_figure(self._figure, site)
        self._message.hide()
