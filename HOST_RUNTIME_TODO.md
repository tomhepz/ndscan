...
This document captures follow-up work for the host-runtime scan model after the first
...
- Prefer these native point policies as the host-runtime core abstraction rather than
...
- Keep legacy `ScanGenerator` classes as a compatibility layer for the old runtime/UI,
  not as the long-term core for the host runtime.
...
- Compile host submission schema directly to native point policies instead of routing
