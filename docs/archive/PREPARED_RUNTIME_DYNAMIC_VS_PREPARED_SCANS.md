# Prepared Runtime: Dynamic vs Prepared Scans
...
- scan submission vs kernel scan
...
The current prepared runtime now uses one nested-scan style:
...
In the current prepared runtime, a prepared child scan already just derives a child site
...
What the newer prepared runtime improves is not that kernel story by itself, but the place
...
The current prepared runtime has already won something important:
