...
practical path forward is a new prepared runtime written alongside the current
...
3. Build a new prepared runtime beside the existing runtime.
...
- add a new prepared scan runner beside the legacy runtime
...
## Proposed prepared runtime model
...
The current prepared runtime has now moved beyond the early sketch above. The main
implemented flow is:
...
flowchart TD
    ...
    RUN --> ANALYSIS[_ScanAnalysisPlan]
...
3. new prepared `ProgramScanRunner`
4. `grid` strategy
5. `zip` strategy
...
The most practical path is a new prepared runtime built around:
...
This section is the concrete implementation plan for the prepared-runtime refactor.
...
Implement a new prepared runtime for ndscan from first principles, then make
...
#### Phase 3: Implement the smallest executable prepared runtime
...
#### Phase 4: Implement the writer and canonical top-level prepared runtime
...
- top-level prepared runtime now uses the same site schema as planned for subscans
...
- top-level plot subscriber can load new prepared runtime datasets
...
3. Add large module-level docstrings marking legacy kernel path as legacy and prepared runtime as canonical.
4. Ensure prepared runtime is the only path new scan modes use.
...
1. top-level scan submission tests
2. subscan tests
...
10. Plot/offline consumers can read the new prepared runtime schema.
...
9. The resulting codebase has one prepared runtime, not two.
...
### Next priorities after the current prepared-runtime milestone

The new prepared runtime now has:
